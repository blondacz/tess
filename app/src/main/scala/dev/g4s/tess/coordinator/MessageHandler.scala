...
import dev.g4s.tess.core.{Actor, ActorFactory, ActorKey, ActorUnitOfWork, CommandMessage, Envelope, Event, Message}
...
class MessageHandler(actorFactory: GenericActorFactory, coordinator: Coordinator) {
  ...
  def handle: PartialFunction[Envelope, Seq[ActorUnitOfWork]] = {
    case env @ Envelope(msg @ CommandMessage(_, ids), trace) if ids.forall(id => actorFactory.idClass.isInstance(id)) =>
      val startingRank = coordinator.lastReactionRank.fold(1L)(_ + 1)
      processMessage(env, startingRank, ids.asInstanceOf[List[actorFactory.ActorIdType]])
    case env @ Envelope(msg, _) if actorFactory.route.isDefinedAt(msg) =>
      val startingRank = coordinator.lastReactionRank.fold(1L)(_ + 1)
      val ids: Seq[actorFactory.ActorIdType] = actorFactory.route(msg)
      processMessage(env, startingRank, ids)
  }

  private def processMessage(env: Envelope, startingRank: Long, ids: Seq[actorFactory.ActorIdType]): Seq[ActorUnitOfWork] = {
    val msg = env.message
      ...
      case ((uows, rank), id) =>
        ...
        val (uow, nextRank) = coordinator.load(loadKey)(actorFactory) match {
          ...
          case Right(None) => // create
            ...
            val uow = ActorUnitOfWork(ActorKey(id, actor.getClass), 1, reactionsSeq, rank, env.trace)
          ...
          case Right(Some((rehydrated, v))) =>
            ...
            val uow = ActorUnitOfWork(ActorKey(id, updatedActor.getClass), v + 1, actorReactions, rank, env.trace)
            coordinator.store(uow, updatedActor).fold(throw _, identity) //FIXME: propagate
